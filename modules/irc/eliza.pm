...
# $Id: eliza.pm 1916 2005-10-06 13:53:01Z brain $
# =============================================

package modules::irc::eliza;

my $chatterbot;

sub init {
	my ($self) = @_;
	$chatterbot = new Chatbot::Eliza 'botnix';
	$chatterbot->memory_on(20);
}
...
sub on_privmsg {
	...
	if (
		($nick ne $main::config{$nid}{nickname}) && ($text =~ /\Q$main::config{$nid}{nickname}\E/i)
	) {
		if ($text =~ /^\Q$main::config{$nid}{nickname}\E\:/i) {
			$text =~ s/\Q$main::config{$nid}{nickname}\E\://i;
		}
                if ($text =~ /^\Q$main::config{$nid}{nickname}\E\,/i) {
                        $text =~ s/\Q$main::config{$nid}{nickname}\E\,//i;
                }
                if ($text =~ /^\Q$main::config{$nid}{nickname}\E\s/i) {
                        $text =~ s/\Q$main::config{$nid}{nickname}\E\s//i;
                }
		if ($target =~ /^\#|\&/) {
			main::send_privmsg($nid,$target,"\002$nick\002: " . $chatterbot->transform($text));
		} else {
			main::send_privmsg($nid,$nick,"\002$nick\002: " . $chatterbot->transform($text));
		}
	}
}
...
sub parse_script_data {
	...
	for (@scriptlines) { 
		...
		for ($entrytype) {   
			...
			/decomp/	and do { 
						die "$0: error parsing script:  decomposition rule with no 
keyword.\n" 
							if $thiskey eq "";
			...
			/reasmb/	and do { 
						die "$0: error parsing script:  reassembly rule with no 
decomposition rule.\n" 
							if $thisdecomp eq "";
...
decomposition rule.\n" 
							if $thisdecomp eq "";
						push @{ $self->{reasmblist_for_memory}->{$thisdecomp} }, 
$entry ;  
						last; 
					};
...
pre: kinda alike
pre: sorta alike
synon: belief feel think believe wish want
synon: family mother mom father dad sister brother wife children child kid girlfriend boyfriend bf gf friend mate
synon: desire want need require give take steal
synon: sad unhappy depressed sick bored angry annoyed
synon: happy elated glad better good smile laugh
synon: cannot can't cant not
synon: everyone everybody nobody noone every1 all none nothing everything
synon: be am is are was were
synon: hello hi yo oi hey
key: xnone -1
  decomp: *
    ...
    reasmb: I'm bored, lets go play on railway lines!
    reasmb: I'm a doctor, not a conversationalist!
    reasmb: I'm a doctor, not an IRC bot!
    reasmb: I'm a doctor, not a lamp post!
key: sorry 0
  decomp: *
    ...
    reasmb: Fine, apologise NOW why don't you, could have said sooner
    reasmb: You already hurt my feelings *sob*
    reasmb: Well i'm sorry too.
    reasmb: Are you REALLY sorry?
key: apologise 0
  decomp: *
    reasmb: goto sorry
key: remember 5
...
key: hello 0
  decomp: *
    ...
    reasmb: Do you own a pet snake?
    reasmb: Can someone give me a live wire to chew?
    reasmb: I'm hungry
    reasmb: Want to play with my slinky?
    reasmb: Does this concern me?
    reasmb: WHAT?! STOP POKING ME!
    reasmb: Oh really...
    reasmb: You must be bored talking to me.
key: computer 50
  decomp: *
    ...
    reasmb: I'm a living person! REALLY!
    reasmb: If only i had a heart.
key: am 0
  decomp: * am i *
    ...
    reasmb: I don't think you're (2)
    reasmb: Yes, you are definitely (2)
    reasmb: You are (2) and it's funny as hell.
    reasmb: goto what
  decomp: * i am *
    reasmb: Wait till i tell all my friends that you are (2)!
    reasmb: If you are (2) what am i???
    reasmb: I am (2) not you!
    reasmb: If you say so.
    reasmb: There is no way you will ever be (2)
    reasmb: Crystal ball say, you are indeed (2)
    reasmb: goto i
  decomp: *
    reasmb: Why do you say 'am' ?
    reasmb: I don't understand that.
    reasmb: SPEAK ENGLISH moron!
key: are 0
  decomp: * are you *
